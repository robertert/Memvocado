...
export default function createScreen() {
  ...
  const [seperator, setSeperator] = useState("Select seperator");
  const [open, setOpen] = useState(false);
  const [listPosition, setListPosition] = useState({ top: 0, left: 0 });
  const [selected, setSelected] = useState(null);
  const [seperator1, setSeperator1] = useState("Select seperator");
  const [open1, setOpen1] = useState(false);
  const [selected1, setSelected1] = useState(null);
  const [listPosition1, setListPosition1] = useState({ top: 0, left: 0 });
  const [text, setText] = useState();

  const triggerRef = useRef(null);
  const triggerRef1 = useRef(null);
  const list1ContainerRef = useRef(null);
  const list2ContainerRef = useRef(null);
  ...
  function openList() {
    list1ContainerRef.current.measure((x, y, width, height, pageX, pageY) => {
  ...
  function openList1() {
    list2ContainerRef.current.measure((x, y, width, height, pageX, pageY) => {
      const conPageX = pageX;
      const conPageY = pageY;
      triggerRef1.current.measure((x, y, width, height, pageX, pageY) => {
        setListPosition1({
          top: pageY + height - conPageY,
          left: pageX - conPageX,
        });
